...
export const getAllSupervisorsController = async (req, res) => {
  try {
    const { page, limit, search, date } = req.query;
    const { sellers, paginationInfo } = await getAllSupervisors({ page, limit, search, date });
    generateResponse(res, 200, true, 'Supervisors fetched successfully', { sellers, paginationInfo });
  } catch (error) {
    generateResponse(res, 500, false, 'Failed to fetch supervisors', null);
  }
};
